...
#define VHIST "Version History\n\n1.0.0:\nInital Version\n\n1.1.0\n- Addet saving your settings\n- Addet App icon\n- Some bugfixes\n\n1.2.0\n- Addet Lobby System\n- Some bugfixes\n\n2.0.0\nSwitch to discord game sdk"
bool vhrst = false;
...
void InitDiscord()
{
	auto result = discord::Core::Create(S64(dckey.c_str()), DiscordCreateFlags_Default, &core);
...
int main()
{
	...
	NImGui::App app("DC-Client (2.0.0)", NImGui::Vec2i(400, 700), NImGui::BORDERLESS | NImGui::NORESIZE | (1 << 4));
	...
	{
		if (lobbysize < 0) lobbysize = 0;
		if (lobbymembers < 0) lobbymembers = 0;
		ImGui::SetNextWindowViewport(ImGui::GetMainViewport()->ID);
		ImGui::Begin("DC-Client (2.0.0)", &rnn, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_MenuBar);
		ImGui::SetWindowSize(ImVec2(app.GetWindowSize().x, app.GetWindowSize().y));
		app.SetWindowPos(NImGui::Vec2i(ImGui::GetWindowPos().x, ImGui::GetWindowPos().y));
		Bar();
		...
		if (ImGui::BeginCombo("Activity", actions_c.c_str()))
		{
			...
			{
				bool is_selected = (actions_c == actions_l[n]); // You can store your selection however you want, outside or inside your objects
				if (ImGui::Selectable(actions_l[n], is_selected))
					actions_c = actions_l[n];
				if (is_selected)
					ImGui::SetItemDefaultFocus();   // You may set the initial focus when opening the combo (scrolling + for keyboard navigation support)
			}
			ImGui::EndCombo(); // only call ImGui::EndCombo() if BeginCombo() returns true!
		}
		...
		if (vhrst)
		{
			ImGui::Begin("Version History", &vhrst, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
			ImGui::Text(VHIST);
			ImGui::End();
		}
		DiscordMain();
